...
void setupHome(mos6502 mpu, uint8_t* memory) {
    ...
    dtext_opt(32 + 66 * 0, 214, C_WHITE, C_BLACK, DTEXT_MIDDLE, DTEXT_MIDDLE, "Run N", -1); // F1
    dtext_opt(32 + 66 * 1, 214, C_WHITE, C_BLACK, DTEXT_MIDDLE, DTEXT_MIDDLE, "Reset", -1); // F2
    dtext_opt(32 + 66 * 2, 214, C_WHITE, C_BLACK, DTEXT_MIDDLE, DTEXT_MIDDLE, "Interrupt", -1); // F3
    dtext_opt(32 + 66 * 3, 214, C_WHITE, C_BLACK, DTEXT_MIDDLE, DTEXT_MIDDLE, "Edit", -1); // F4
    dtext_opt(32 + 66 * 4, 214, C_WHITE, C_BLACK, DTEXT_MIDDLE, DTEXT_MIDDLE, "Range", -1); // F5
    dtext_opt(32 + 66 * 5, 214, C_WHITE, C_BLACK, DTEXT_MIDDLE, DTEXT_MIDDLE, "Load", -1); // F6
        ...
        for (int addrY = 0; addrY < 0x10 + 1; addrY++) {
            ...
            if (addrX == 0) { // Address
                ...
                if (addrY == 0) { // Least Significant Nibble
                    ...
                    if (startAddress + ((addrY - 1) << 4) + addrX - 1 == programCounter)
                    {
                        ...
                        dprint_opt(TOPLEFT_VAL_X + VAL_OFFSET_X * (addrX - 1),
                            ...
                            DTEXT_LEFT,
                            DTEXT_TOP,
                            "%02X", memory[startAddress + ((addrY - 1) << 4) + addrX - 1]
                        );
                    }
                    else {
                        drect(TOPLEFT_VAL_X + VAL_OFFSET_X * (addrX - 1) - 1, 
